...
private fun Modifier.plHotspot(x: Float, y: Float, w: Float, h: Float, sw: Float, sh: Float) =
    this.offset((sw * x / 1080f).dp, (sh * y / 2400f).dp)
        .size((sw * w / 1080f).dp, (sh * h / 2400f).dp)

@Composable
fun PlaylistsScreen(onBack: () -> Unit) {
    ReferenceScreen(R.drawable.reference_playlists_final) { sw, sh ->
        Box(Modifier.plHotspot(20f, 20f, 120f, 140f, sw, sh).clickable(onClick = onBack))
    }
}
